...
function updateAcceptorsForSSL() {
  ...
  if sslEnabled ; then
    ...
    echo "=xxx===keystore filepath: $keyStorePath"
  ...
  ;;
    ...
    #debug
    echo "===dump broker.xml==="
    cat ${instanceDir}/etc/broker.xml
  fi
}
